...
int main(int argc, char **argv) {
  ...
  unsigned int N =
      atoi(argv[1]); // Máximo N =2^32-1=4294967295 (sizeof(unsigned int) = 4 B)

#ifdef VECTOR_LOCAL
  double v1[N], v2[N],
      v3[N]; // Tamaño variable local en tiempo de ejecución ...
  ...
  v1 = (double *)malloc(N *
                        sizeof(double)); // malloc necesita el tamaño en bytes
  v2 = (double *)malloc(
      N * sizeof(double)); // si no hay espacio suficiente malloc devuelve NULL
...
#pragma omp section
    {
      for (tercer_trozo = (N / 2); tercer_trozo < ((3 * N) / 4);
           tercer_trozo++) {
...
// Imprimir resultado de la suma y el tiempo de ejecución
  ...
  if (N == 8 || N == 11) {
    ...
    printf("Tiempo(seg.):%11.9f\t / Tamaño Vectores:%u\n/ "
           ...
           tiempo, N, v1[0], v2[0], v3[0], N - 1, N - 1, N - 1, v1[N - 1],
           v2[N - 1], v3[N - 1]);
  }
#endif
#ifdef VECTOR_DYNAMIC
